...
之后我们还要用自定义用户配置命令自动选择 `silo` ，这样我们平常使用 `denote` 命令的时候默认将笔记放到 `01Index` 中， 当我们切换到 `02Area` 时，我们这时使用 `denote` 相关命令，笔记就会自动存放到当前的 `silo` 中了。
...
### denote-org-store-link-to-heading 用户选项
